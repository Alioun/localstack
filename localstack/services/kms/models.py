...
def _encrypt(key_id, key_material, plaintext, encryption_context):
    if plaintext == b"":
...
def _decrypt(key_material, ciphertext, encryption_context):
    aad = _serialize_encryption_context(encryption_context=encryption_context)
...
class KmsKey:
    ...
    def encrypt(self, plaintext: str, encryption_context: Dict = {}):
        return _encrypt(
    ...
    def decrypt(self, ciphertext: str, encryption_context: Dict = {}):
    ...
    def _construct_sign_verify_kwargs(self, signing_algorithm: SigningAlgorithmSpec):
        kwargs = {}
    ...
    def _populate_metadata(
        ...
        if self.metadata.get("MultiRegion"):
            key_id_prefix = "mrk-"
        self.metadata["KeyId"] = key_id_prefix + str(uuid.uuid4())
...
def get_key_id_from_any_id(some_id: str, store: KmsStore):
    alias_name = None
    key_id = None
